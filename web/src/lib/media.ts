...
function getThumbnailUrlRaw(
  ...
  if (isImage(file)) {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        const { w, h } = (() => {
          const scale = Math.min(maxSize.w / img.naturalWidth, maxSize.h / img.naturalHeight, 1);
        ...
        canvas.height = h;
        const ctx = canvas.getContext('2d');
        if (!ctx) { URL.revokeObjectURL(objectUrl); reject(new Error('Canvas failed')); return; }
        ctx.drawImage(img, 0, 0, w, h);
